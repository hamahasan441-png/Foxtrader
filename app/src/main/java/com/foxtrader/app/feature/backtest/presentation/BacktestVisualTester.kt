...
fun BacktestVisualTester(
    ...
    LabCard {
        SectionTitle("Visual Strategy Tester")
  ...
  ReplayButton(if (state.replayPlaying) "Pause" else "Play", Modifier.weight(0.9f), viewModel::toggleVisualReplayPlay)
  ReplayButton("+1", Modifier.weight(0.7f)) { viewModel.stepVisualReplay(1) }
  ...
  Text("W ${projection.wins} · L ${projection.losses} · BE/T ${projection.ties}", color = MaterialTheme.colorScheme.onSurface, fontSize = 11.sp)
  Text(String.format(Locale.US, "Win %.1f%%", projection.winRate), color = FoxAmber50, fontSize = 11.sp)
  Text(String.format(Locale.US, "P&L %.2f", projection.netPnL), color = if (projection.netPnL >= 0.0) FoxBullishText else FoxBearishText, fontSize = 11.sp)
        }
        Text(
...
@Composable
private fun ReplayButton(label: String, modifier: Modifier, onClick: () -> Unit) {
    Button(
        onClick = onClick,
...
private fun ReplayCandleCanvas(
    ...
    val visibleMarkers = markers.filter { marker ->
        marker.entryIndex in start..safeCursor || (marker.exitIndex != null && marker.exitIndex in start..safeCursor)
    }
    ...
    Canvas(modifier = modifier.background(Color(0xFF0D1015))) {
        ...
        fun y(price: Double): Float = ((maxPrice - price) / priceSpan * size.height).toFloat().coerceIn(0f, size.height)

        repeat(4) { row ->
  val gy = size.height * row / 4f
  drawLine(Color(0x223A4657), Offset(0f, gy), Offset(size.width, gy), 1f)
        }
  ...
  drawLine(color, Offset(cx, highY), Offset(cx, lowY), 1.2f)
  val top = minOf(openY, closeY)
  val h = abs(closeY - openY).coerceAtLeast(1.5f)
  drawRect(color, Offset(cx - bodyW / 2f, top), Size(bodyW, h))
        }
  ...
  if (marker.entryIndex in start..safeCursor) {
      ...
      val c = if (marker.direction == Direction.BULLISH) FoxBullishText else FoxBearishText
      val path = Path().apply {
          if (marker.direction == Direction.BULLISH) {
              moveTo(cx, cy - 8f); lineTo(cx - 6f, cy + 3f); lineTo(cx + 6f, cy + 3f)
          } else {
              moveTo(cx, cy + 8f); lineTo(cx - 6f, cy - 3f); lineTo(cx + 6f, cy - 3f)
          }
          close()
  ...
  if (exit != null && exitPrice != null && exit in start..safeCursor) {
      val c = when (marker.outcome) {
          "WIN" -> FoxBullishText
          "LOSS" -> FoxBearishText
          else -> FoxAmber50
      }
      drawCircle(c, radius = 4.5f, center = Offset(x(exit), y(exitPrice)))
  }
        }
